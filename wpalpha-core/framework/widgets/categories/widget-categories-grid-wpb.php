...
$params = array_merge( alpha_wpb_filter_element_params( $params, 'wpb_' . ALPHA_NAME . '_categories_grid' ), alpha_get_wpb_design_controls(), alpha_get_wpb_extra_controls() );

vc_map(
	array(
		'name'            => esc_html__( 'Product Categories Grid', 'alpha-core' ),
		'base'            => 'wpb_' . ALPHA_NAME . '_categories_grid',
		'icon'            => 'alpha-icon alpha-icon-cat-grid',
		'class'           => 'alpha_categories alpha_categories_grid',
		'content_element' => true,
		'controls'        => 'full',
		'category'        => ALPHA_DISPLAY_NAME,
		'description'     => esc_html__( 'Create alpha product categories with grid layout.', 'alpha-core' ),
		'params'          => $params,
	)
);

add_filter( 'vc_autocomplete_wpb_' . ALPHA_NAME . '_categories_grid_category_ids_callback', 'alpha_wpb_shortcode_product_category_id_callback', 10, 1 );
add_filter( 'vc_autocomplete_wpb_' . ALPHA_NAME . '_categories_grid_category_ids_render', 'alpha_wpb_shortcode_product_category_id_render', 10, 1 );

if ( class_exists( 'WPBakeryShortCode' ) ) {
	eval( 'class WPBakeryShortCode_WPB_' . ucfirst( ALPHA_NAME ) . '_Categories_Grid extends WPBakeryShortCode {}' );
}
