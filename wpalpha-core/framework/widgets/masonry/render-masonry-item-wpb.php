...
// Preprocess
$wrapper_attrs = array(
	'class' => 'alpha-masonry-item-container ' . $atts['shortcode_class'] . $atts['style_class'],
);
...
$settings = array_merge( array(), alpha_wpb_convert_responsive_values( 'creative_width', $atts, 0 ) );
$settings = array_merge( $settings, alpha_wpb_convert_responsive_values( 'creative_height', $atts, 'preset' ) );
$settings = array_merge( $settings, alpha_wpb_convert_responsive_values( 'creative_order', $atts, '' ) );

$grid = array();

global $alpha_wpb_creative_layout;

if ( isset( $alpha_wpb_creative_layout ) && ! vc_is_inline() ) {
	$idx = $alpha_wpb_creative_layout['index'];
	if ( $idx < count( $alpha_wpb_creative_layout['preset'] ) ) {
		foreach ( $alpha_wpb_creative_layout['preset'][ $idx ] as $key => $value ) {
			if ( 'h' == $key ) {
				continue;
			}
...
if ( 'preset' == $settings['creative_height_xl'] ) {
	if ( ! vc_is_inline() ) {
		$grid['h'] = $idx < count( $alpha_wpb_creative_layout['preset'] ) ? $alpha_wpb_creative_layout['preset'][ $idx ]['h'] : '1-3';
	}
...
if ( isset( $alpha_wpb_creative_layout ) && ! vc_is_inline() ) {
	$alpha_wpb_creative_layout['layout'][ $idx ] = $grid;
	$alpha_wpb_creative_layout['index']          = ++$idx;
}
