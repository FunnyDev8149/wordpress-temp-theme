...
// Preprocess
$wrapper_attrs = array(
	'class' => 'alpha-carousel-container ' . $atts['shortcode_class'] . $atts['style_class'],
);
...
// Responsive columns
$settings = array_merge( $settings, alpha_wpb_convert_responsive_values( 'col_cnt', $atts, 0 ) );
if ( ! $settings['col_cnt'] ) {
...
$extra_attrs  = ' data-slider-options="' . esc_attr(
	json_encode(
		alpha_get_slider_attrs( $settings, $col_cnt, $thumb_hash )
	)
) . '"';

echo '<div class="slider-frame">';
echo '<div class="' . $extra_class . '" ' . alpha_strip_script_tags( $extra_attrs ) . '>';
echo do_shortcode( $atts['content'] );
echo '</div>';
echo '</div>';

?>
<?php if ( true == $settings['show_dots'] && 'thumb' == $settings['dots_type'] ) : ?>
