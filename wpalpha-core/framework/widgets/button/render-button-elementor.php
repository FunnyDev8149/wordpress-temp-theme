...
if ( empty( $label ) ) {
	$label = esc_html__( 'Click here', 'alpha-core' );
}
...
if ( isset( $play_btn ) && 'yes' == $play_btn ) {
	...
	echo '<div class="' . $class . '" role="button"' . ( $options ? ( ' data-elementor-open-lightbox="yes" data-elementor-lightbox="' . esc_attr( json_encode( $options ) ) . '"' ) : '' ) . '>' . alpha_strip_script_tags( $label ) . '</div>';
