<?php
/**
 * Products Layout Banner Item Element
 *
 * @author     FunnyWP
 * @package    WP Alpha Core FrameWork
 * @subpackage Core
 * @since      1.0
 */

$params = array(
	esc_html__( 'Layout', 'alpha-core' )   => array(
		array(
			'type'        => 'alpha_number',
			'param_name'  => 'item_no',
			'std'         => 1,
			'heading'     => esc_html__( 'Insert At', 'alpha-core' ),
			'description' => esc_html__( 'Input item index where this banner should be inserted before.', 'alpha-core' ),
		),
		array(
			'type'       => 'alpha_heading',
			'tag'        => 'h3',
			'label'      => esc_html__( 'Creative Layout Options', 'alpha-core' ),
			'param_name' => 'creative_item_heading',
		),
		array(
			'type'        => 'alpha_number',
			'heading'     => esc_html__( 'Banner Column Size', 'alpha-core' ),
			'param_name'  => 'item_col_span',
			'std'         => '{"xl":"2","unit":"","xs":"","sm":"","md":"","lg":""}',
			'responsive'  => true,
			'description' => esc_html__( 'Control column size of banner in this layout. This option works only for creative layout.', 'alpha-core' ),
			'dependency'  => array(
				'element' => 'layout_type',
				'value'   => 'creative',
			),
			'selectors'   => array(
				'.creative-grid > {{WRAPPER}}' => 'grid-column-end: span {{VALUE}}',
			),
		),
		array(
			'type'        => 'alpha_number',
			'heading'     => esc_html__( 'Banner Row Size', 'alpha-core' ),
			'param_name'  => 'item_row_span',
			'std'         => '{"xl":"1","unit":"","xs":"","sm":"","md":"","lg":""}',
			'responsive'  => true,
			'description' => esc_html__( 'Control row size of banner in this layout. This option works only for creative layout.', 'alpha-core' ),
			'dependency'  => array(
				'element' => 'layout_type',
				'value'   => 'creative',
			),
			'selectors'   => array(
				'.creative-grid > {{WRAPPER}}' => 'grid-row-end: span {{VALUE}}',
			),
		),
	),
	esc_html__( 'General', 'alpha-core' )  => array(
		'alpha_wpb_banner_general_controls',
	),
	esc_html__( 'Effect', 'alpha-core' )   => array(
		'alpha_wpb_banner_effect_controls',
	),
	esc_html__( 'Parallax', 'alpha-core' ) => array(
		'alpha_wpb_banner_parallax_controls',
	),
	esc_html__( 'Video', 'alpha-core' )    => array(
		'alpha_wpb_banner_video_controls',
	),

);

$params = array_merge( alpha_wpb_filter_element_params( $params ), alpha_get_wpb_design_controls(), alpha_get_wpb_extra_controls() );

vc_map(
	array(
		'name'                    => esc_html__( 'Banner Inner Products Layout', 'alpha-core' ),
		'base'                    => 'wpb_' . ALPHA_NAME . '_products_banner_item',
		'icon'                    => 'alpha-icon alpha-icon-banner',
		'class'                   => 'alpha_products_banner_item',
		'controls'                => 'full',
		'category'                => ALPHA_DISPLAY_NAME,
		'description'             => esc_html__( 'Create alpha banner item inside products layout.', 'alpha-core' ),
		'as_parent'               => array( 'only' => 'wpb_' . ALPHA_NAME . '_banner_layer' ),
		'as_child'                => array( 'only' => 'wpb_' . ALPHA_NAME . '_products_layout' ),
		'show_settings_on_create' => true,
		'js_view'                 => 'VcColumnView',
		'params'                  => $params,
	)
);

if ( class_exists( 'WPBakeryShortCode' ) ) {
	class WPBakeryShortCode_wpb_alpha_products_banner_item extends WPBakeryShortCodesContainer {

	}
}
