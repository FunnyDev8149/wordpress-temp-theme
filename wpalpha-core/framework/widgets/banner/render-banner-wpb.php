...
extract( // @codingStandardsIgnoreLine
	shortcode_atts(
		array(
			'wrap_with'       => '',
			'banner_image'    => defined( 'ALPHA_ASSETS' ) ? ( ALPHA_ASSETS . '/images/placeholders/banner-placeholder.jpg' ) : '',
			'full_screen'     => '',
			'min_height'      => '300',
			'max_height'      => '',
			'hover_effect'    => '',
...
// Preprocess
$wrapper_attrs = array(
	'class' => 'alpha-banner-container ' . $atts['shortcode_class'] . $atts['style_class'],
);
...
if ( 'banner' == $atts['shortcode'] ) {
	...
	$alpha_products_banner_items[ count( $alpha_products_banner_items ) - 1 ]['banner_class'] = $wrapper_attrs['class'];
}
...
if ( $banner_image ) {
	...
	if ( is_numeric( $banner_image ) ) {
		...
		$html .= '<img src="' . esc_url( $banner_image ) . '" alt="' . esc_attr__( 'Banner', 'alpha-core' ) . '"/>';
	}
	$html .= '</figure>';
}
