...
// Preprocess
$wrapper_attrs = array(
	'class' => 'alpha-tab-container ' . $atts['shortcode_class'] . $atts['style_class'],
);
...
if ( ! vc_is_inline() ) {
	...
	if ( ! empty( $alpha_wpb_tab ) ) {
		for ( $i = 0; $i < count( $alpha_wpb_tab ); $i ++ ) {
			echo '<li class="nav-item">';
			echo '<a class="nav-link' . ( 0 == $i ? ' active' : '' ) . '" href="#">' . alpha_strip_script_tags( $alpha_wpb_tab[ $i ] ) . '</a>';
			echo '</li>';
		}
		$alpha_wpb_tab = array();
	}
...
echo alpha_strip_script_tags( $atts['content'] );
echo '</div>';
echo '</div>';
?>
</div>
