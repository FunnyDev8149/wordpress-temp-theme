...
class Alpha_Single_Product_Builder extends Alpha_Base {
	...
	public function __construct() {
		$this->widgets = apply_filters(
			...
			array(
				'image',           // @feature: fs_sp_image
				'navigation',      // @feature: fs_sp_navigation
				'title',           // @feature: fs_sp_title
				'meta',            // @feature: fs_sp_meta
				'rating',          // @feature: fs_sp_rating
				'price',           // @feature: fs_sp_price
				'flash_sale',      // @feature: fs_sp_flashsale
				'excerpt',         // @feature: fs_sp_excerpt
				'cart_form',       // @feature: fs_sp_cartform
				'share',           // @feature: fs_sp_share
				'data_tab',        // @feature: fs_sp_datatab
				'fbt',             // @feature: fs_sp_fbt
				'linked_products', // @feature: fs_sp_linked
				'vendor_products', // @feature: fs_sp_vendorproducts
				'counter',         // @feature: fs_sp_counter
				'compare',         // @feature: fs_sp_compare
				'wishlist',        // @feature: fs_sp_wishlist
				'tags',            // @feature: fs_sp_tags
			)
		);
		// setup builder
	...
	public function find_preview() {
		global $post;
		if ( ( wp_doing_ajax() && isset( $_REQUEST['action'] ) && 'elementor_ajax' == $_REQUEST['action'] ) || ( doing_action( 'wp' ) && ALPHA_NAME . '_template' == get_post_type() && 'product' == get_post_meta( $post->ID, ALPHA_NAME . '_template_type', true ) ||
			doing_action( 'init' ) && ( alpha_is_elementor_preview() || alpha_is_wpb_preview() ) &&
			isset( $_GET['post'] ) && ALPHA_NAME . '_template' == get_post_type( (int) $_GET['post'] ) && 'product' == get_post_meta( (int) $_GET['post'], ALPHA_NAME . '_template_type', true ) ) ) {
	...
	public function run_template( $run ) {
		...
		if ( $post && ALPHA_NAME . '_template' == $post->post_type && 'product_layout' == get_post_meta( $post->ID, ALPHA_NAME . '_template_type', true ) ) {
			...
			if ( isset( $alpha_layout['single_product_type'] ) && 'builder' == $alpha_layout['single_product_type'] &&
				! empty( $alpha_layout['single_product_template'] ) && is_numeric( $alpha_layout['single_product_template'] ) ) {
