<?php
/**
 * Alpha Responsive Callback
 *
 * adds responsive control for element option
 * follow below example of alpha_responsive control
 *
 *
 * @since 1.0
 *
 ...
 * @return string
 */
function alpha_responsive_callback( $settings, $value ) {
	$param_name = isset( $settings['param_name'] ) ? $settings['param_name'] : '';
	$type       = isset( $settings['type'] ) ? $settings['type'] : '';
	$form_value = $value;
	$value      = ! empty( $value ) ? json_decode( $value, true ) : array();
	$class      = 'alpha-wpb-responsive-container';
...
vc_add_shortcode_param( 'alpha_responsive', 'alpha_responsive_callback', ALPHA_CORE_PLUGINS_URI . '/wpb/params/responsive.js' );
