<?php
/**
 * Alpha WPBakery ColorGroup Callback
 *
 * adds colorgroup control for element option
 * follow below example of alpha_color_group control
 *
 * array(
 *      'type'       => 'alpha_color_group',
 *      'heading'    => __( 'Colors', 'alpha-core' ),
 ...
 * @return string
 */
function alpha_color_group_callback( $settings, $value ) {
	...
	$available_choices = array(
		'color'            => esc_html__( 'Color', 'alpha-core' ),
		'background-color' => esc_html__( 'Background Color', 'alpha-core' ),
		'border-color'     => esc_html__( 'Border Color', 'alpha-core' ),
	);
	$choices           = isset( $settings['choices'] ) ? $settings['choices'] : array( 'color', 'background-color', 'border-color' );

	$class = 'alpha-wpb-color-group';
	$html  = '';
	...
	foreach ( $available_selectors as $selector ) {
		if ( in_array( $selector, $selectors ) ) {
				$html .= '<li class="nav-item"><a href="#" class="nav-link' . ( 0 == $idx ++ ? ' active' : '' ) . '" data-pane-id="' . $selector . '">' . sprintf( esc_html__( '%s', 'alpha-core' ), ucwords( $selector ) ) . '</a></li>';
		}
	}
...
vc_add_shortcode_param( 'alpha_color_group', 'alpha_color_group_callback', ALPHA_CORE_PLUGINS_URI . '/wpb/params/color-group.js' );
