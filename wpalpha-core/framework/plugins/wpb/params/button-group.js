 ...
 * @since 1.0
 */
jQuery( '.alpha-wpb-button-group .alpha-responsive-toggle' ).on( 'click', function ( e ) {
    var $this = jQuery( this );
    $this.parent().toggleClass( 'show' );
} );

if ( undefined == alpha_core_vars.wpb_editor || undefined == alpha_core_vars.wpb_editor.alpha_button_group_included || true != alpha_core_vars.wpb_editor.alpha_button_group_included ) {
    jQuery( document.body ).on( 'click', '.alpha-wpb-button-group .alpha-responsive-span li', function ( e ) {
        var $this = jQuery( this ),
    ...
    } ).off( 'responsive_changed', '.alpha-wpb-button-group .alpha-responsive-span li' ).on( 'responsive_changed', '.alpha-wpb-button-group .alpha-responsive-span li', function ( e ) {
        var $this = jQuery( this ),
            $dropdown = $this.closest( '.alpha-responsive-dropdown' ),
            $toggle = $dropdown.find( '.alpha-responsive-toggle' ),
            $control = $dropdown.parent(),
    ...
    } ).on( 'click', '.alpha-wpb-button-group .options-wrapper li', function ( e ) {
        var $this = jQuery( this ),
            value = $this.attr( 'attr-value' ),
            $control = $this.closest( '.alpha-wpb-button-group' ),
            $form = $control.next();
    ...
    if ( undefined == alpha_core_vars.wpb_editor ) {
        alpha_core_vars.wpb_editor = {
            alpha_button_group_included: true,
        }
    } else {
        alpha_core_vars.wpb_editor.alpha_button_group_included = true;
    }
}