 ...
 * array(
 *      'type'        => 'alpha_number',
 *      'heading'     => __( 'Icon Spacing', 'alpha-core' ),
 ...
 * @return string
 */
function alpha_number_callback( $settings, $value ) {
	...
	$class         = 'alpha-wpb-number-container';
	...
	if ( ! empty( $units ) ) {
		...
		<input type="<?php echo esc_attr( $with_unit ? 'text' : 'number' ); ?>"
			...
			?>
			<input type="<?php echo esc_attr( $with_unit ? 'text' : 'number' ); ?>"
			class="alpha-wpb-number simple-value"
...
vc_add_shortcode_param( 'alpha_number', 'alpha_number_callback', ALPHA_CORE_PLUGINS_URI . '/wpb/params/number.js' );
