...
// WP Bakery
define( 'ALPHA_CORE_WPB', ALPHA_CORE_PLUGINS . '/wpb' );
define( 'ALPHA_CORE_WPB_URI', ALPHA_CORE_PLUGINS_URI . '/wpb' );

if ( ! class_exists( 'Alpha_WPB' ) ) :
	class Alpha_WPB extends Alpha_Base {
		...
		public function __construct() {
			require_once alpha_core_framework_path( ALPHA_CORE_WPB . '/core-wpb-functions.php' );

			self::$params = apply_filters(
				'alpha_wpb_params',
				array(
					'button-group',
					'color-group',
			...
			self::$dimensions = apply_filters(
				'alpha_wpb_dimensions',
				array(
					'top'    => '{{TOP}}',
					'right'  => '{{RIGHT}}',
					'bottom' => '{{BOTTOM}}',
			...
			// registers alpha addons for wpb page builder including studio, resize, template condition
			...
			if ( alpha_is_wpb_preview() ) {
				add_filter( 'alpha_builder_addon_html', array( $this, 'add_addon_html_to_vars' ) );
			...
			foreach ( $this::$params as $param ) {
				require_once alpha_core_framework_path( ALPHA_CORE_WPB . '/params/' . $param . '.php' );
			}

			require_once alpha_core_framework_path( ALPHA_CORE_WPB . '/existing.php' );

			if ( vc_is_inline() ) {
				alpha_remove_all_admin_notices();
			}
			...
			add_action( 'wp_enqueue_scripts', array( $this, 'add_wpb_css' ), 30 );
		 ...
		 * @since 1.0
		 */
		public function init_alpha_icons() {
			require_once alpha_core_framework_path( ALPHA_CORE_WPB . '/lib/icons.php' );
			add_action(
				'admin_enqueue_scripts',
				function() {
					if ( alpha_is_wpb_preview() && defined( 'ALPHA_VERSION' ) ) {
		...
		public function editor_scripts() {
			wp_enqueue_style( 'alpha-js-composer-editor', alpha_core_framework_uri( '/plugins/wpb/assets/wpb-admin' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array(), ALPHA_CORE_VERSION );
			wp_enqueue_script( 'alpha-wpb-admin-js', alpha_core_framework_uri( '/plugins/wpb/assets/wpb-admin' . ALPHA_JS_SUFFIX ), array(), ALPHA_CORE_VERSION, true );
			wp_localize_script(
				'alpha-wpb-admin-js',
				'alpha_wpb_admin_vars',
				apply_filters(
					'alpha_core_wpb_admin_localize_vars',
					array(
		 ...
		 * add_addon_list_html
		 *
		 * registers alpha addons for wpb page builder including below items
		 * - studio
		 * - edit area resize
		 * - template condition
		 * - popup options
		...
		public function add_addon_list_html( $list ) {
			$html  = '<li class="alpha-wpb-addons">';
			$html .= '<a class="vc_icon-btn alpha-wpb-addons-trigger" title="' . ALPHA_DISPLAY_NAME . esc_html( ' Addons', 'alpha-core' ) . '"><span class="vc-composer-icon alpha-mini-logo"></span></a>';
			$html .= '<ul class="dropdown alpha-addons-dropdown">';

			$html .= '<li><a id="wpb-alpha-studio-trigger" class="alpha-wpb-addon-item"><i class="fas fa-layer-group"></i>' . ALPHA_DISPLAY_NAME . esc_html( ' Studio', 'alpha-core' ) . '</a></li>';

			$post_type          = get_post_type( get_the_ID() );
			$post_type_category = get_post_meta( get_the_ID(), ALPHA_NAME . '_template_type', true );

			if ( ALPHA_NAME . '_template' == $post_type && 'popup' == $post_type_category ) {
				$html .= '<li><a id="alpha-popup-options" class="alpha-wpb-addon-item"><i class="far fa-sun"></i>' . esc_html( 'Popup Options', 'alpha-core' ) . '</a></li>';
					...
					if ( false !== strpos( $path, 'editors/popups/class-vc-post-settings.php' ) ) {
						add_filter(
							...
							function( $vars ) use ( $id, $template ) {
								...
								ob_start();
								include alpha_core_framework_path( ALPHA_CORE_WPB . '/panels/' . $template );
		...
		public function save_custom_panel_options( $post_id ) {
			if ( isset( $post_id ) && is_numeric( $post_id ) ) { // post save
				// save popup options
				if ( ! empty( $_POST['alpha_popup_options'] ) ) {
					update_post_meta( $post_id, 'popup_options', $_POST['alpha_popup_options'] );
				}
			} else { // ajax save
				$post_id = intval( vc_post_param( 'post_id' ) );
				vc_user_access()->checkAdminNonce()->validateDie()->wpAny( 'edit_posts', 'edit_pages' )->validateDie()->canEdit( $post_id )->validateDie();
				...
				// save popup options
				$popup_options = vc_post_param( 'alpha_popup_options' );
		...
		public function add_global_css() {
			...
			$used_wpb_shortcodes = function_exists( 'alpha_get_option' ) ? alpha_get_option( 'used_wpb_shortcodes', false ) : get_theme_mod( 'used_wpb_shortcodes', false );
			if ( ! empty( $used_wpb_shortcodes ) && ! alpha_is_wpb_preview() ) {
				$upload_dir = wp_upload_dir();
				$upload_url = $upload_dir['baseurl'];
				$css_file   = $upload_dir['basedir'] . '/alpha_styles/js_composer.css';
				if ( file_exists( $css_file ) ) {
					$inline_styles = wp_styles()->get_data( 'js_composer_front', 'after' );
					wp_deregister_style( 'js_composer_front' );
					wp_dequeue_style( 'js_composer_front' );
					wp_enqueue_style( 'js_composer_front', $upload_url . '/alpha_styles/js_composer.css', array(), ALPHA_VERSION );
			...
			if ( defined( 'ALPHA_VERSION' ) ) {
				wp_enqueue_style( 'alpha-animation', ALPHA_ASSETS . '/vendor/animate/animate.min.css' );
			}
		}
		...
		public function add_wpb_css() {
			wp_enqueue_style( 'alpha-wpb-style', alpha_core_framework_uri( '/plugins/wpb/assets/wpb' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ) );
			...
			if ( ! empty( $alpha_layout['used_blocks'] ) ) {
				foreach ( $alpha_layout['used_blocks'] as $block_id => $enqueued ) {
					if ( ! ( alpha_is_wpb_preview() && isset( $_REQUEST['post_id'] ) && $_REQUEST['post_id'] == $block_id ) ) {
		...
		public function save_google_fonts( $id ) {
			...
			if ( ! empty( $fonts ) ) {
				update_post_meta( $id, 'alpha_vc_google_fonts', rawurlencode( json_encode( $fonts ) ) );
			}
		}
			...
			foreach ( $shortcodes[2] as $index => $tag ) {
				...
				if ( ! empty( $shortcode['params'] ) ) {
					foreach ( $shortcode['params'] as $param ) {
						if ( 'alpha_typography' === $param['type'] ) {
		...
		public function add_shortcodes( $shortcodes = array(), $path = 'elements' ) {
			...
			require_once alpha_core_framework_path( ALPHA_CORE_WPB . '/partials/extra.php' );
			require_once alpha_core_framework_path( ALPHA_CORE_WPB . '/partials/design-option.php' );
			...
			foreach ( $shortcodes as $shortcode ) {
				...
				// Get prefix and shortcode_filename
				...
				if ( ! file_exists( alpha_core_framework_path( ALPHA_CORE_FRAMEWORK_PATH . $prefix . '/widget-' . $shortcode_filename . '-wpb.php' ) ) ) {
					continue;
				}

				$callback = function( $atts, $content = null ) use ( $shortcode, $prefix, $shortcode_filename ) {
					...
					if ( defined( 'VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG' ) ) {
						if ( isset( $atts['css'] ) ) {
							$style_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $atts['css'], ' ' ), 'wpb_' . ALPHA_NAME . '_' . $shortcode, $atts );
						}
					}
					// Frontend editor
					if ( isset( $_REQUEST['vc_editable'] ) && ( true == $_REQUEST['vc_editable'] ) ) {
						$style_array = $this->alpha_generate_shortcode_css( 'wpb_' . ALPHA_NAME . '_' . $shortcode, $atts );
						...
						// Google Fonts
						$weights = array();
						$sc      = WPBMap::getShortCode( 'wpb_' . ALPHA_NAME . '_' . $shortcode );
						if ( ! empty( $sc['params'] ) ) {
							foreach ( $sc['params'] as $param ) {
								if ( ! empty( $param['type'] ) && 'alpha_typography' === $param['type'] ) {
					...
					$atts['tag']             = 'wpb_' . ALPHA_NAME . '_' . $shortcode;
					...
					if ( ! empty( $sc['params'] ) ) {
						$atts['shortcode_class'] = ' wpb_custom_' . alpha_get_global_hashcode( $atts, 'wpb_' . ALPHA_NAME . '_' . $shortcode, $sc['params'] );
					}

					ob_start();
					require alpha_core_framework_path( ALPHA_CORE_FRAMEWORK_PATH . $prefix . '/render-' . $shortcode_filename . '-wpb.php' );
					echo ob_get_clean();

					return ob_get_clean();
				};

				if ( ! shortcode_exists( 'wpb_' . ALPHA_NAME . '_' . $shortcode ) ) {
					add_shortcode( 'wpb_' . ALPHA_NAME . '_' . $shortcode, $callback );
				}

				add_action(
					'vc_after_init',
					function() use ( $shortcode, $left, $right, $path, $prefix, $shortcode_filename ) {
						require_once alpha_core_framework_path( ALPHA_CORE_FRAMEWORK_PATH . $prefix . '/widget-' . $shortcode_filename . '-wpb.php' );
					}
				);
			}
		}
		...
		public function parse_shortcodes_custom_css( $content ) {
			...
			foreach ( $shortcodes[2] as $index => $tag ) {
				// Get attributes
				$atts = shortcode_parse_atts( trim( $shortcodes[3][ $index ] ) );
				$css  = array_merge_recursive( $css, $this->alpha_generate_shortcode_css( $tag, $atts ) );
			}
		 ...
		 * @return array
		 */
		public function alpha_generate_shortcode_css( $tag, $atts ) {
			$css = array();
			if ( defined( 'WPB_VC_VERSION' ) ) {
				$shortcode = WPBMap::getShortCode( $tag );
				...
				if ( isset( $shortcode['params'] ) && ! empty( $shortcode['params'] ) ) {
					$shortcode_class = '.wpb_custom_' . alpha_get_global_hashcode( $atts, $tag, $shortcode['params'] );
					...
					foreach ( $shortcode['params'] as $param ) {
						if ( isset( $param['selectors'] ) && ( isset( $atts[ $param['param_name'] ] ) || isset( $param['std'] ) ) ) {
							...
							foreach ( $param['selectors'] as $key => $value ) {
								...
								if ( 'alpha_number' == $param['type'] && ! empty( $param['units'] ) && is_array( $param['units'] ) ) {
								...
								} elseif ( 'alpha_dimension' == $param['type'] ) {
									$saved_value      = str_replace( '``', '"', $saved_value );
									$dimension_values = json_decode( $saved_value, true );
								} elseif ( 'alpha_typography' == $param['type'] ) {
									$saved_value = str_replace( '``', '"', $saved_value );
									...
									if ( ! empty( $saved_value['size'] ) && alpha_check_units( $saved_value['size'] ) ) {
										$typography .= 'font-size:' . alpha_check_units( $saved_value['size'] ) . ';';
									}
								...
								if ( ! empty( $param['responsive'] ) && $param['responsive'] ) {
									...
									if ( 'alpha_dimension' == $param['type'] ) {
										...
										foreach ( $breakpoints as $breakpoint => $width ) {
											...
											foreach ( $this::$dimensions as $dimension => $pattern ) {
												if ( isset( $dimension_values[ $dimension ][ $breakpoint ] ) ) {
													$temp = alpha_check_units( $dimension_values[ $dimension ][ $breakpoint ] );
													if ( ! $temp ) {
														$temp_value = preg_replace( '/([^;]*)(\{\{' . strtoupper( $dimension ) . '\}\})([^;]*)(;*)/', '', $temp_value );
													} else {
														$temp_value = str_replace( $pattern, $temp, $temp_value );
										...
										if ( ! empty( $responsive_values['xl'] ) || ( isset( $responsive_values['xl'] ) && '0' === $responsive_values['xl'] ) ) {
											if ( ! empty( $param['with_units'] ) && $param['with_units'] ) {
												$responsive_values['xl'] = alpha_check_units( $responsive_values['xl'] );
										...
										foreach ( $breakpoints as $breakpoint => $width ) {
											if ( ! empty( $param['with_units'] ) && $param['with_units'] ) {
												$responsive_values[ $breakpoint ] = alpha_check_units( $responsive_values[ $breakpoint ] );
											}
									...
									if ( ! empty( $param['with_units'] ) && $param['with_units'] ) {
										$saved_value = alpha_check_units( $saved_value );

										if ( ! $saved_value ) {
											continue;
										}
									}
									if ( 'alpha_dimension' == $param['type'] ) { // Dimension
										foreach ( $this::$dimensions as $dimension => $pattern ) {
											$temp = alpha_check_units( $dimension_values[ $dimension ]['xl'] );
											if ( ! $temp ) {
									...
									} elseif ( 'alpha_color_group' == $param['type'] ) { // Color Group
									...
									} elseif ( 'alpha_typography' == $param['type'] && ! empty( $typography ) ) {
										if ( empty( $css[ str_replace( '{{WRAPPER}}', $shortcode_class, $value ) ] ) ) {
		...
		public function init_vc_custom_styles() {
			...
			$css = alpha_strip_script_tags( ob_get_clean() );
			if ( $css ) {
				if ( is_singular() && alpha_get_option( 'lazyload' ) && ! vc_is_inline() ) {
					global $post;
		...
		public function enqueue_vc_editor_preview_style() {
			if ( vc_is_inline() ) {
				wp_enqueue_style( 'alpha-js-composer-editor-preview', alpha_core_framework_uri( '/plugins/wpb/assets/wpb-preview' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array(), ALPHA_CORE_VERSION );
			}
		}
	}
endif;

Alpha_WPB::get_instance();
