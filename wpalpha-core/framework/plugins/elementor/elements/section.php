...
class Alpha_Element_Section extends Elementor\Element_Section {
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			array(
				'label'     => esc_html__( 'Creative Grid', 'alpha-core' ),
				'tab'       => Controls_Manager::TAB_LAYOUT,
				'condition' => array(
					'use_as' => 'creative',
				),
		...
		$this->start_controls_section(
			'section_accordion',
			array(
				'label'     => esc_html__( 'Accordion', 'alpha-core' ),
				'tab'       => Controls_Manager::TAB_LAYOUT,
				'condition' => array(
					'use_as' => 'accordion',
				),
		...
		$this->start_controls_section(
			'alpha_video_section',
			array(
				'label'     => esc_html__( 'Video', 'alpha-core' ),
				'tab'       => Controls_Manager::TAB_LAYOUT,
				'condition' => array(
					'use_as'              => 'banner',
		...
		$this->add_control(
			...
			array(
				...
				'placeholder' => esc_html__( 'Enter your URL', 'alpha-core' ) . ' (Vimeo)',
		...
		$this->update_control(
			...
			$this->start_controls_section(
				...
				array(
					'label' => ALPHA_DISPLAY_NAME . esc_html__( ' Typography', 'alpha-core' ),
					'tab'   => Controls_Manager::TAB_STYLE,
				)
			);
	...
	protected function content_template() {
		...
		<# if ( 'banner' == settings.use_as && 'yes' != settings.parallax && settings.background_image.url ) { #>
			<figure class="banner-img" style="background-color: {{ settings.background_color }}">
				<img src="{{ settings.background_image.url }}" alt="<?php esc_attr_e( 'Banner', 'alpha-core' ); ?>">
			</figure>
		<# } #>
			...
			<?php } else { ?>
				...
				<?php } if ( 'tab' == $settings['use_as'] ) : // add tab navs ?>
					...
					<?php foreach ( $alpha_section['tab_data'] as $idx => $data ) : ?>
						...
						?>
						<li class="nav-item <?php echo $data['icon'] ? 'nav-icon-' . $data['icon_pos'] : ''; ?>"><a class="nav-link<?php echo esc_attr( 0 == $idx ? ' active' : '' ); ?>" href="<?php echo esc_attr( $data['id'] ); ?>"><?php echo alpha_strip_script_tags( $html ); ?></a></li>
					<?php endforeach; ?>
					</ul>
				<?php endif; ?>
				<?php if ( $this->legacy_mode ) { ?>
