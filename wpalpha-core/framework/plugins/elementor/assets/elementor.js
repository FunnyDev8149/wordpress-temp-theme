...
( function ( $ ) {
    ...
    themeAdmin.themeElementorPreview.initWidgets = function () {
        ...
        // Widget for banner
        elementorFrontend.hooks.addAction( 'frontend/element_ready/' + alpha_vars.theme + '_widget_banner.default', function ( $obj ) {
        ...
        elementorFrontend.hooks.addAction( 'frontend/element_ready/' + alpha_vars.theme + '_widget_progressbars.default', function ( $obj ) {
            theme.progressbarInit( $obj.find( '.progress-wrapper' ), true );
        } );
        ...
        elementorFrontend.hooks.addAction( 'frontend/element_ready/' + alpha_vars.theme + '_widget_flipbox.default', function ( $obj ) {
            ...
            if ( !window.themeElementorAdmin ) {
                return;
            }

            if ( !window.themeElementorAdmin.activeSection ) {
                return;
            }

            var section = window.themeElementorAdmin.activeSection;
            var isBackSide = -1 !== [ 'section_back_side_content' ].indexOf( section );

            if ( isBackSide ) {
                $obj.find( '.flipbox' ).addClass( 'hover' );
            } else {
                $obj.find( '.flipbox' ).removeClass( 'hover' );
            }
        } );
