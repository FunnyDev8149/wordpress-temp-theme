...
if ( ! class_exists( 'Alpha_Vendors' ) ) {
	/**
	 * Alpha Vendors Class
	 *
	 * @since 1.0
	 */
	class Alpha_Vendors extends Alpha_Base {
		...
		public function __construct() {
			add_action( 'wp_ajax_alpha_get_vendors', array( $this, 'get_vendors' ) );
			add_action( 'wp_ajax_nopriv_alpha_get_vendors', array( $this, 'get_vendors' ) );

			if ( function_exists( 'alpha_get_option' ) && alpha_get_option( 'resource_disable_dokan' ) && ! is_user_logged_in() ) {
		...
		public static function get_top_selling_vendors( $limit, $period ) {
			...
			$cache_key = 'alpha-best-seller-' . $limit;
			// $sellers   = wp_cache_get( $cache_key, 'alpha-elementor-widget' );
			$sellers = false;
			$results = array();
			...
			if ( false == $sellers ) {
							...
							"SELECT seller_id, total_sale
                                FROM (
                                    SELECT vendor_id AS seller_id, SUM(total_sales) AS total_sale
                                        ...
                                        WHERE {$wpdb->prefix}wc_order_stats.status = 'wc-completed' AND" . alpha_query_time_range_filter( 'wp_posts', 'post_date', $period ) .
										" GROUP BY seller_id
                                ) AS wpfmo
							...
							"SELECT seller_id, total_sale
                                FROM (
                                    SELECT vendor_id AS seller_id, SUM(total_due) AS total_sale
                                        ...
                                        WHERE {$wpdb->prefix}wc_order_stats.status = 'wc-completed' AND" . alpha_query_time_range_filter( $wpdb->prefix . 'wc_order_stats', 'date_created', $period ) .
										" GROUP BY seller_id
                                ) AS wcv
				...
				wp_cache_set( $cache_key, $sellers, 'alpha-elementor-widget', 3600 * 6 );
			}
			...
			return apply_filters( 'alpha_get_top_selling_vendors', $results );
		}
		...
		public static function get_top_rating_vendors( $limit = 5 ) {
			...
			$cache_key = 'alpha-top-rated-seller-' . $limit;
			$sellers   = wp_cache_get( $cache_key, 'alpha-elementor-widget' );
			$results   = array();
			...
			return apply_filters( 'alpha_get_top_rating_vendors', $results );
		}
...
/**
 * alpha_query_time_range_filter
 *
 * Query helper function
 *
 * @since 1.0
 */
if ( ! function_exists( 'alpha_query_time_range_filter' ) ) {
	function alpha_query_time_range_filter( $table_handler, $time, $interval = 'all' ) {
		$sql = ' 1=1';
		switch ( $interval ) {
			case 'year':
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_get_vendors' ) ) {
	function alpha_get_vendors( $args = array(), $orderby = 'registered', $limit = -1 ) {
		$query_args = array(
			'role'       => array( 'seller' ),
			'number'     => $limit,
			'offset'     => 0,
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_get_vendor_total_sale' ) ) {
	function alpha_get_vendor_total_sale( $vendor_id, $tbl_handler, $field_name = 'vendor_id' ) {
		global $wpdb;

		$cache_key  = 'alpha_vendor_total_sale_' . $vendor_id;
		$total_sale = wp_cache_get( $cache_key, 'alpha-total-sale' );
		...
		$total_sale = $total_sales ? $total_sales[0] : esc_html__( 'N/A', 'alpha-core' );

		wp_cache_set( $cache_key, $total_sale, 'alpha-elementor-widget', 3600 * 6 );
		wp_reset_postdata();
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_get_sellers' ) ) {
	function alpha_get_sellers( $limit = 5, $orderby = 'registered' ) {
		...
		return apply_filters( 'alpha_get_sellers', $results );
	}
}
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_get_dokan_vendor_info' ) ) {
	function alpha_get_dokan_vendor_info( $vendor ) {

		$store_info = dokan_get_store_info( $vendor['id'] );
		if ( $store_info ) {
			$vendor['store_name']     = isset( $store_info['store_name'] ) ? esc_html( $store_info['store_name'] ) : __( 'N/A', 'alpha-core' );
			...
			if ( ! isset( $vendor['total_sale'] ) ) {
				$result = alpha_get_vendor_total_sale( $vendor['id'], 'dokan_orders', 'seller_id' );
				if ( $result ) {
		...
		return apply_filters( 'alpha_get_dokan_vendors', $vendor );
	}
}


/**
 * alpha_get_wcfm_vendor_info
 *
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_get_wcfm_vendor_info' ) ) {
	function alpha_get_wcfm_vendor_info( $vendor ) {
		...
		if ( $store_user ) {
			...
			if ( ! isset( $vendor['total_sale'] ) ) {
				$result = alpha_get_vendor_total_sale( $vendor['id'], 'wcfm_marketplace_orders' );
				if ( $result ) {
					$vendor['total_sale'] = is_numeric( $result->total_sale ) ? $result->total_sale : 0;
				}
			}
		...
		return apply_filters( 'alpha_get_wcfm_vendor_info', $vendor );
	}
}


/**
 * alpha_get_wcmp_vendor_info
 *
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_get_wcmp_vendor_info' ) ) {
	function alpha_get_wcmp_vendor_info( $vendor ) {
		// phpcs:disable WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
		global $WCMp;
		...
		if ( $wcmp_vendor ) {
			...
			if ( ! isset( $vendor['total_sale'] ) ) {
				$result = alpha_get_vendor_total_sale( $vendor['id'], 'wcmp_vendor_orders' );
				if ( $result ) {
					$vendor['total_sale'] = is_numeric( $result->total_sale ) ? $result->total_sale : 0;
				}
			}
		...
		return apply_filters( 'alpha_get_wcmp_vendor_info', $vendor );
	}
}


/**
 * alpha_get_wc_vendor_info
 *
 * Get WC Vendor info
 *
 * @param array $vendor
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_get_wc_vendor_info' ) ) {
	function alpha_get_wc_vendor_info( $vendor ) {
		global $wpdb;
		...
		if ( $vendor['store_name'] ) {
			...
			if ( ! isset( $vendor['total_sale'] ) ) {
				$result = alpha_get_vendor_total_sale( $vendor['id'], 'pv_commission' );
				if ( $result ) {
					$vendor['total_sale'] = is_numeric( $result->total_sale ) ? $result->total_sale : 0;
				}
			}
		...
		return apply_filters( 'alpha_get_wc_vendor_info', $vendor );
	}
}
