	...
	class Alpha_Product_Video_Popup extends Alpha_Base {
		...
		public function add_meta_boxes( $meta_boxes ) {
			$meta_boxes[] = array(
				...
				'fields'     => array(
					array(
						'name' => esc_html__( 'Video from Library', 'alpha-core' ),
						'id'   => 'alpha_product_video_thumbnail',
						'type' => 'video',
						'std'  => false,
					),
					array(
						'name' => esc_html__( 'Video URL', 'alpha-core' ),
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_script( 'alpha-product-video-popup', alpha_core_framework_uri( '/addons/product-video-popup/product-video-popup' . ALPHA_JS_SUFFIX ), array( 'alpha-theme' ), ALPHA_VERSION, true );
		}
		...
		public function get_video_viewer_btn( $buttons ) {
			return $buttons . '<button class="product-gallery-btn open-product-video-viewer ' . ALPHA_ICON_PREFIX . '-icon-movie" title="' . esc_html__( 'Product Video Thumbnail', 'alpha-core' ) . '"></button>';
		}
	}
}
