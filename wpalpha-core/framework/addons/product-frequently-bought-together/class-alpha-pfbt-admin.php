...
if ( ! class_exists( 'Alpha_Product_Frequently_Bought_Together_Admin' ) ) {
	...
	class Alpha_Product_Frequently_Bought_Together_Admin extends Alpha_Base {
		...
		public function admin_enqueue_script() {
			...
			wp_enqueue_style( 'alpha-product-fbt-admin', alpha_core_framework_uri( '/addons/product-frequently-bought-together/fbt-admin.min.css' ), array(), ALPHA_VERSION );
			wp_enqueue_script( 'alpha-product-fbt-admin', alpha_core_framework_uri( '/addons/product-frequently-bought-together/fbt-admin' . ALPHA_JS_SUFFIX ), array( 'jquery-core' ), ALPHA_VERSION, true );
		}
								...
								<p class="form-field" <?php echo esc_html( $data ); ?>>
									...
									switch ( $option_value['type'] ) {
										case 'select-products':
											?>
											<select class="wc-product-search" multiple="multiple" style="width: 50%;" id="alpha_fbt_ids" name="alpha_fbt_ids[]" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-action="alpha_woocommerce_json_search_fbt" data-exclude="<?php echo intval( $thepostid ); ?>">
												<?php
												$product_ids = $value;
