		...
		public function add_product_data_panel() {
			...
			<div id="swatch_product_options" class="panel wc-metaboxes-wrapper woocommerce_options_panel hidden">
				...
				if ( ! count( $attributes ) ) :
					...
					?>
					<div class="inline error notice woocommerce-message"><p><?php echo sprintf( esc_html__( 'Product Attribute should have list( Advanced Swatch ) type. Please see the attribute type. %1$sClick here%2$s', 'alpha-core' ), '<a target="_blank" href="' . admin_url( 'edit.php?post_type=product&page=product_attributes' ) . '">', '</a>' ); ?></p></div>
		...
		public function enqueue_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'title'       => esc_html( 'Choose an image', 'alpha-core' ),
					'button_text' => esc_html( 'Use image', 'alpha-core' ),
				)
			);
		}
				...
				foreach ( $this->attribute_taxonomies as $tax ) {
					if ( wc_attribute_taxonomy_name( $tax->attribute_name ) == $taxonomy ) {
						...
						<div class="form-field term-swatch-label-wrap">
							...
							<p class="description"><?php echo esc_html__( 'This option is added by Our Theme. This label will be shown on attribute swatches.', 'alpha-core' ); ?></p>
						</div>
						<div class="form-field term-swatch-color-wrap">
							...
							<p class="description"><?php echo esc_html__( 'This option is added by Our Theme. Each attribute swatch will be filled with this color.', 'alpha-core' ); ?></p>
						</div>
						<div class="form-field term-swatch-image-wrap">
							...
							<div class="clear"></div>
							<p class="description"><?php echo esc_html__( 'This option is added by Our Theme. This image will be shown on attribute swatches.', 'alpha-core' ); ?></p>
						</div>
						<?php
					}
				}
