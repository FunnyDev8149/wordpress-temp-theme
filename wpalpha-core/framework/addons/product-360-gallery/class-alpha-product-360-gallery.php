	...
	class Alpha_Product_360_Gallery extends Alpha_Base {
		...
		public function add_customize_fields( $fields ) {
			...
			$fields['cs_360_gallery_about_desc']  = array(
				...
				'default' => '<p class="options-custom-description option-feature-description"><img class="description-image" src="' . ALPHA_ASSETS . '/images/admin/customizer/360gallery.jpg' . '" alt="Theme Option Descrpition Image"></p>',
			);
			...
			$meta_boxes[] = array(
				...
				'fields'     => array(
					array(
						...
						'name' => esc_html__( 'Upload Images', 'alpha-core' ),
						'type' => 'image_advanced',
					),
				),
			);
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_script( 'alpha-product-360-gallery', alpha_core_framework_uri( '/addons/product-360-gallery/product-360-gallery' . ALPHA_JS_SUFFIX ), array( 'alpha-theme' ), ALPHA_CORE_VERSION, true );
		}
		...
		public function get_degree_viewer_btn( $buttons ) {
			return $buttons . '<button class="product-gallery-btn open-product-degree-viewer ' . ALPHA_ICON_PREFIX . '-icon-rotate-3d" title="' . esc_html__( 'Product 360 Degree Gallery', 'alpha-core' ) . '"></button>';
		}
	}
}
