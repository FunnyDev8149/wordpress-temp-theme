...
if ( $vendor ) {
	if ( 'section' == apply_filters( 'alpha_single_product_data_tab_type', 'tab' ) ) : ?>
		<h2 class="title-wrapper title-underline">
			<span class="title"><?php echo esc_html( alpha_get_option( 'product_vendor_info_title' ) ); ?></span>
		</h2>
	<?php endif; ?>
	...
	<div class="row">
		<div class="col-lg-5 mb-4">
			<figure class="vendor-banner">
				<?php echo apply_filters( 'alpha_lazyload_images', wp_get_attachment_image( $banner, 'full' ) ); ?>
			</figure>
		</div>
		<div class="col-lg-7 pl-lg-6 mb-4">
			<div class="vendor-user">
				...
				<div>
					...
					if ( ! $rating['total_rating'] ) {
						$html = __( 'No ratings found yet!', 'alpha' );
					} else {
						// translators: %1$s represents rating value, %2$d represents 5
						$text  = sprintf( __( 'Rated %1$s out of %2$d', 'alpha' ), $rating['avg_rating'], number_format( 5 ) );
						...
						'<span class="text">' . sprintf( esc_html__( '(%s Reviews)', 'alpha' ), $rating['total_rating'] ) . '</span>';
					}
			...
			<ul class="list-unstyled list sp-vendor-info">
				...
				<?php if ( null != apply_filters( 'wcmp_vendor_lists_single_button_text', $vendor->page_title ) ) { ?>
					<li class="store-name">
						<span><?php esc_html_e( 'Store Name:', 'alpha' ); ?></span>
						<span class="details">
				...
				<?php if ( $vendor->get_formatted_address() ) { ?>
					<li class="store-address">
						<span><?php esc_html_e( 'Address:', 'alpha' ); ?></span>
						<span class="details">
				...
				<?php if ( $vendor->phone ) { ?>
					<li class="store-phone">
						<span><?php esc_html_e( 'Phone:', 'alpha' ); ?></span>
						<span class="details">
							<?php printf( '<a href="tel:%1$s">%1$s</a>', esc_html( $vendor->phone ) ); ?>
						</span>
					</li>
				<?php } ?>

			</ul>
			<a href="<?php echo esc_url( $vendor->permalink ); ?>" class="btn btn-link btn-underline"><?php esc_html_e( 'Visit Store', 'alpha' ); ?><i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-long-arrow-<?php echo is_rtl() ? 'left' : 'right'; ?>"></i></a>
		</div>
	</div>
	...
	if ( $vendor->description ) {
		echo '<div class="vendor-description pt-3 pb-3">' . alpha_strip_script_tags( $vendor->description ) . '</div>';
	}
