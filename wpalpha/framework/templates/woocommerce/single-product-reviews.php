...
<div id="reviews" class="woocommerce-Reviews">
	...
	<div id="comments">
		<div class="row">
			<div class="col-md-4 mb-4">
				...
				<?php if ( $review_offcanvas && 'no' === get_option( 'woocommerce_review_rating_verification_required' ) || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
					<a class="btn btn-dark submit-review-toggle" href="#"><?php echo esc_html__( 'Submit Review', 'alpha' ); ?></a>
				<?php endif; ?>
			</div>
				...
				<div id="review_form_wrapper" <?php echo ( $review_offcanvas ? 'class="offcanvas"' : '' ); ?>>
					...
					<div id="review_form" <?php echo ( $review_offcanvas ? 'class="offcanvas-content"' : '' ); ?>>
						...
						$comment_form = array(
							...
							'title_reply'         => have_comments() ? esc_html__( 'Submit Your Review', 'woocommerce' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'woocommerce' ), get_the_title() ),
							/* translators: %s is product title */
				...
				<?php if ( have_comments() ) : ?>
					...
					<ul id="alpha_comment_tabs" class="alpha-comment-tabs nav nav-tabs tab-nav-solid" role="tablist" data-post_id = "<?php the_ID(); ?>">
						<li class="nav-item" role="tab" aria-controls="commentlist">
							<span data-href="#commentlist" class="nav-link active" data-mode="all"><?php esc_html_e( 'Show all', 'alpha' ); ?></span>
						</li>
					...
					<div class="tab-content tab-templates">
						...
						if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
							...
							$args  = apply_filters(
								...
								array(
									'echo'      => false,
									'prev_text' => '<i class="w-icon-long-arrow-left"></i> ' . esc_html__( 'Prev', 'alpha' ),
									'next_text' => esc_html__( 'Next', 'alpha' ) . ' <i class="w-icon-long-arrow-right"></i>',
								)
							);
		...
		<?php if ( have_comments() && ! alpha_get_option( 'product_review_offcanvas' ) ) : ?>
				<?php
				// @start feature: fs_addon_product_helpful_comments
				if ( alpha_get_feature( 'fs_addon_product_helpful_comments' ) ) {
					?>
