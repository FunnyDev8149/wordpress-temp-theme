...
} elseif ( alpha_is_shop() && ! alpha_wc_get_loop_prop( 'widget' ) ) {
...
// For Category widget
// if ( 'product-category-group' == alpha_wc_get_loop_prop( 'widget' ) )
{
...
if ( false !== array_search( 'filter-categories', $wrapper_class ) ) {
	...
	if ( alpha_wc_get_loop_prop( 'show_all_filter' ) && isset( $category_class ) ) {
		echo '<li class="category-wrap nav-filter-clean"><div class="product-category ' . implode( ' ', $category_class ) . '"><a href="#" class="active"><h3 class="woocommerce-loop-category__title">' . esc_html__( 'All', 'alpha' ) . '</h3></a></div></li>';
	}
}
