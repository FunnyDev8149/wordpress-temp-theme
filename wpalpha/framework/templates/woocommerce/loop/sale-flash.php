	...
	if ( is_array( $show_labels ) && count( $show_labels ) ) {
		...
		if ( $product->is_on_sale() && in_array( 'sale', $show_labels ) ) {
			...
			if ( $percentage ) {
				...
				$html .= '<label class="product-label label-sale">' . sprintf( esc_html__( '%d%% off', 'alpha' ), $percentage ) . '</label>';
			}
		}

		// Out of stock label
		if ( in_array( 'stock', $show_labels ) && 'outofstock' == $product->get_stock_status() ) {
			$html .= '<label class="product-label label-stock">' . esc_html__( 'Out of Stock', 'alpha' ) . '</label>';
		}
	}
}
