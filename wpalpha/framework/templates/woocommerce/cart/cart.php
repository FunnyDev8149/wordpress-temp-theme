						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
								...
								if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
									echo alpha_strip_script_tags( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'woocommerce' ) . '</p>', $product_id ) );
								}
								?>
								</td>
					...
					<tr>
						<td colspan="6" class="actions pr-0 pt-4 pl-0 pb-0">
							<div class="cart-actions mb-8">
								<a href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>" class="btn btn-dark btn-rounded btn-icon-left continue-shopping mb-4 <?php echo is_rtl() ? 'ml-auto' : 'mr-auto'; ?>"><?php echo is_rtl() ? '' : '<i class="' . ALPHA_ICON_PREFIX . '-icon-long-arrow-left"></i>'; ?><?php esc_html_e( 'Continue Shopping', 'alpha' ); ?><?php echo is_rtl() ? '<i class="' . ALPHA_ICON_PREFIX . '-icon-long-arrow-right"></i>' : ''; ?></a>
								<?php if ( alpha_get_option( 'cart_show_clear' ) ) : ?>
									<button type="submit" class="btn btn-rounded btn-outline btn-default btn-border-thin mb-4 mr-2 clear-cart-button" name="clear_cart" value="<?php esc_attr_e( 'Clear cart', 'alpha' ); ?>"><?php esc_html_e( 'Clear cart', 'alpha' ); ?></button>
								<?php endif; ?>
								<?php if ( ! alpha_get_option( 'cart_auto_update' ) ) : ?>
									<button type="submit" class="btn btn-rounded btn-outline btn-default btn-border-thin wc-action-btn<?php echo ( alpha_get_option( 'cart_auto_update' ) ? ' d-none' : '' ) . ( is_rtl() ? ' mr-2' : '' ); ?> mb-4" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'alpha' ); ?>"><?php esc_html_e( 'Update cart', 'alpha' ); ?></button>
								<?php endif; ?>
							</div>
