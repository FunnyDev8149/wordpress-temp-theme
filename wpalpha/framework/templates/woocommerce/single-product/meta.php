...
if ( is_array( $brands ) && count( $brands ) ) {
	foreach ( $brands as $brand_id => $brand_name ) {
		...
		if ( $brand_image ) {
			...
			$brand_html .= '<span>' . esc_html__( 'Brand: ', 'alpha' ) . '<a href="' . esc_url( get_term_link( $brand_id, 'product_brand' ) ) . '" title="' . esc_attr( $brand_name ) . '">' . esc_html( $brand_name ) . '</a></span>';
		}
	}
}
		...
		<?php if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) : ?>
			...
			<span class="sku_wrapper">
				...
				<span class="sku">
					<?php
					$sku = $product->get_sku();
					echo alpha_escaped( $sku ) ? $sku : esc_html__( 'N/A', 'woocommerce' );
					?>
				</span>
			</span>
