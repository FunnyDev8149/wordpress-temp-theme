...
@media screen and (max-width: 480px) {
	#wcfmmp-store .header_right {
		background: <?php echo alpha_escaped( $mob_wcfmmp_header_background_color ); ?>;
	}
}
</style>		
...
<div id="wcfmmp-store" class="wcfmmp-single-store-holder <?php echo alpha_escaped( $wcfm_store_wrapper_class ); ?>">
	<div id="wcfmmp-store-content" class="wcfmmp-store-page-wrap woocommerce" role="main">
		<div class="wcfmmp-store-info-area p-relative">
			...
			$WCFMmp->template->get_template(
				'store/wcfmmp-view-store-banner.php',
				array(
			...
			if ( apply_filters( 'wcfmmp_is_allow_legacy_header', false ) ) {
				...
				$WCFMmp->template->get_template(
					'store/wcfmmp-view-store-header.php',
					array(
						'store_user' => $store_user,
						'store_info' => $store_info,
					)
