...
<div class="dokan-profile-frame-wrapper">
					...
					<div class="profile-info">
						...
						<ul class="dokan-store-info">
							<?php if ( ! dokan_is_vendor_info_hidden( 'address' ) && isset( $store_address ) && ! empty( $store_address ) ) { ?>
								<li class="dokan-store-address"><i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-map-marker"></i>
							...
							<?php if ( ! dokan_is_vendor_info_hidden( 'phone' ) && ! empty( $store_user->get_phone() ) ) { ?>
								<li class="dokan-store-phone">
									<i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-phone"></i>
							...
							<li class="dokan-store-rating">
								<i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-star-full"></i>
							...
							<?php if ( 'on' == $show_store_open_close && 'yes' == $dokan_store_time_enabled ) : ?>
								<li class="dokan-store-open-close">
									<i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-cart"></i>
									<?php
								...
								<ul class="store-social social-icons">
									<?php foreach ( $social_fields as $key => $field ) { ?>
										<?php if ( ! empty( $social_info[ $key ] ) ) { ?>
											<li>
												<a class="social-icon framed use-hover social-<?php echo esc_attr( $field['icon'] ); ?>" href="<?php echo esc_url( $social_info[ $key ] ); ?>" target="_blank"><i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-<?php echo esc_attr( $field['icon'] ); ?>"></i></a>
											</li>
										<?php } ?>
									<?php } ?>
								</ul>
	...
	<?php if ( $store_tabs ) { ?>
		<div class="dokan-store-tabs mt-4 <?php echo esc_attr( $no_banner_class_tabs ); ?>">
			<ul class="dokan-list-inline br-3">
				<?php foreach ( $store_tabs as $key => $tab ) { ?>
					<?php if ( $tab['url'] ) : ?>
						<li><a class="font-weight-semi-bold" href="<?php echo esc_url( $tab['url'] ); ?>"><?php echo esc_html( $tab['title'] ); ?></a></li>
					<?php endif; ?>
				<?php } ?>
