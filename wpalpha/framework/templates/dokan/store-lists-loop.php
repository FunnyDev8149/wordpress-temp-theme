		...
		<?php if ( $sellers['users'] ) : ?>
								...
								<div class="store-data-container">
									<div class="store-data">
										...
										<?php if ( ! dokan_is_vendor_info_hidden( 'phone' ) && $store_phone ) { ?>
											<p class="store-phone">
												<i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-phone" aria-hidden="true"></i> <?php echo esc_html( $store_phone ); ?>
											</p>
										<?php } ?>
									...
									<div class="store-action mt-2">
										...
										?>
										<a href="<?php echo esc_url( $store_url ); ?>" class="btn btn-link btn-dark btn-underline"><?php esc_html_e( 'Visit Store', 'dokan-lite' ); ?><i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-long-arrow-<?php echo is_rtl() ? 'left' : 'right'; ?>"></i></a>
									</div>
								</div>
							</div>
			...
			if ( $num_of_pages > 1 ) {
				...
				if ( $page_links ) {
					...
					echo alpha_strip_script_tags( $pagination_links ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
				}
