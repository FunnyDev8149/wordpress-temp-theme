...
<div class="row">
	<div class="col-lg-6 mb-4">
		<figure class="vendor-banner">
			<?php echo apply_filters( 'alpha_lazyload_images', wp_get_attachment_image( $store_info['banner'], 'full' ) ); ?>
		</figure>
	</div>
		...
		<div class="vendor-user">
			...
			<div>
				...
				if ( ! $rating['count'] ) {
					...
					'<span class="text">' . sprintf( esc_html__( '(%s Reviews)', 'alpha' ), $rating['count'] ) . '</span>';
				}
		...
		<ul class="list-unstyled list sp-vendor-info">
			...
			<?php if ( ! dokan_is_vendor_info_hidden( 'address' ) && ! empty( $store_info['address'] ) ) { ?>
				<li class="store-address">
					...
					<span class="details">
						...
						echo alpha_strip_script_tags( str_replace( '<br/>', esc_html__( ', ', 'alpha' ), $address ) );
						?>
					</span>
				</li>
			<?php } ?>
			...
			<?php if ( ! dokan_is_vendor_info_hidden( 'phone' ) && ! empty( $store_info['phone'] ) ) { ?>
				<li class="store-phone">
					...
					<span class="details">
						<?php printf( '<a href="tel:%1$s">%1$s</a>', esc_html( $store_info['phone'] ) ); ?>
					</span>
				</li>
			<?php } ?>
...
if ( $description ) {
	echo '<div class="vendor-description pt-3 pb-3">' . alpha_strip_script_tags( $description ) . '</div>';
}
?>
