...
<?php if ( $r->have_posts() ) : ?>
	...
	while ( $r->have_posts() ) :
		...
		<li class="product-list-sm product product-loop product-wrap">
			<a class="product-media" href="<?php echo esc_url( get_permalink( dokan_get_prop( $product, 'id' ) ) ); ?>" title="<?php echo esc_attr( $product->get_title() ); ?>">
			...
			<div class="product-details">
				...
				<?php
					echo '<span class="price">' . alpha_escaped( $product->get_price_html() ) . '</span>'; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
				?>
			</div>
		</li>
		<?php endwhile; ?>
