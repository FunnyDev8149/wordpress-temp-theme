...
<div class="dokan-store-open-close">
	<?php foreach ( $dokan_store_time as $day => $value ) : ?>
		...
		<div class="open-close-day <?php echo esc_attr( $day ) . '-time'; ?>">
			<label><?php echo esc_html( dokan_get_translated_days( $day ) ); ?></label>
			<?php echo sprintf( __( '<span> %1$s %2$s %3$s </span>', 'dokan-lite' ), $is_open ? esc_attr( ucfirst( $value['opening_time'] ) ) : '', $to, $is_open ? esc_attr( ucfirst( $value['closing_time'] ) ) : '' ); // phpcs:ignore ?>
		</div>
	<?php endforeach; ?>
</div>
