...
class Alpha_WCMP extends Alpha_Base {
	...
	public function __construct() {
		...
		// compatibility with 3.7.8
		add_filter( 'wcmp_load_default_vendor_list', '__return_true' );
		add_filter( 'wcmp_load_default_vendor_store', '__return_true' );

		//Change template path.
		add_filter( 'wcmp_locate_template', array( $this, 'correct_template' ), 10, 2 );
		// Enqueue Alpha-WCMP script
		...
		add_filter( 'alpha_vendor_store_sidebar_has_content', array( $this, 'has_sidebar_contents' ) );

		// Add abuse link
		add_action( 'alpha_after_product_summary', array( $this, 'add_abuse_link' ) );

		// add tabs to vendor store page
		add_action( 'woocommerce_before_shop_loop', array( $this, 'vendor_store_tab_start' ) );
		add_action( 'alpha_after_shop_loop_end', array( $this, 'vendor_store_tab_end' ) );

		// enqueue wcmp product related style for preventing style broken issue becuase of wcmp product btn in homepage
		add_action( 'alpha_eqnueue_product_widget_related_scripts', array( $this, 'enqueue_wcmp_product_scripts' ) );
	...
	public function init_vendor_settings() {
		...
		if ( alpha_get_option( 'product_hide_vendor_tab' ) ) {
			if ( function_exists( 'alpha_clean_filter' ) ) {
				alpha_clean_filter( 'woocommerce_product_tabs', array( $WCMp->product, 'product_vendor_tab' ) );
			}
		} else {
	...
	public function register_sidebar() {
		...
		register_sidebar(
			array(
				'name'          => esc_html__( 'WCMP Store Sidebar', 'alpha' ),
	...
	/**
	 * Set sidebar of alpha layout as wcmp store sidebar
	 *
	 * @since 1.0
	 */
	public function set_store_sidebar() {
		...
		if ( $this->is_store_page() ) {
			$alpha_layout['left_sidebar'] = 'wcmp-store-sidebar';
		}
	}
	...
	public function hide_default_dashboard_link( $arg = '' ) {
		...
		return '<a class="d-none" href="' . $dashboard_page_link . '">' . __( 'Dashboard - manage your account here', 'alpha' ) . '</a>';
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'alpha-wcmp-style', alpha_framework_uri( '/plugins/wcmp/wcmp' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array(), ALPHA_VERSION );
	}
	...
	public function add_abuse_link() {
		...
		if ( apply_filters( 'alpha_is_single_product_widget', false ) ) {
			return;
		}
	...
	public function vendor_store_tab_start() {
		...
		<div class="tab tab-nav-boxed tab-nav-underline tab-nav-sm-center">
			<ul class="nav nav-tabs">
				<li class="nav-item"><a class="nav-link active" href="#vendor-products"><?php echo esc_html__( 'Products', 'alpha' ); ?></a></li>
				<li class="nav-item"><a class="nav-link" href="#vendor-reviews"><?php echo esc_html__( 'Reviews', 'alpha' ); ?></a></li>
			</ul>
	...
	public function add_sold_by_to_loop() {
		...
		if ( $vendor ) {
			$sold_by_text = alpha_get_option( 'sold_by_label' ) ? alpha_get_option( 'sold_by_label' ) : apply_filters( 'wcmp_sold_by_text', __( 'Sold By', 'dc-woocommerce-multi-vendor' ), $post->ID );
			?>
			<div class="alpha-sold-by-container">
				<span class="sold-by-lable"><?php echo esc_html( $sold_by_text ); ?></span>
	...
	public function correct_template( $template, $template_name ) {
		...
		if ( empty( $template ) ) {
			$framework_template_path = ALPHA_PATH . '/framework/' . ALPHA_PART . '/dc-product-vendor/' . $template_name;
