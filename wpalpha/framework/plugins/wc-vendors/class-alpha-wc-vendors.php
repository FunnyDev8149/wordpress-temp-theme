...
class Alpha_WC_Vendors extends Alpha_Base {
	...
	public function __construct() {
		...
		if ( true == alpha_get_option( 'product_hide_vendor_tab' ) && class_exists( 'WCV_Vendor_Shop' ) ) {
			if ( function_exists( 'alpha_clean_filter' ) ) {
				alpha_clean_filter( 'woocommerce_product_tabs', array( 'WCV_Vendor_Shop', 'seller_info_tab' ) );
			}
		} else {
		...
		add_filter( 'wc_get_template', array( $this, 'correct_template' ), 10, 5 );
		...
		add_filter( 'alpha_get_layout', array( $this, 'set_store_sidebar' ) );
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'alpha-wc-vendors-style', alpha_framework_uri( '/plugins/wc-vendors/wc-vendors' . ( is_rtl() ? '-rtl' : '' ) . '.min.css' ), array( 'alpha-theme' ), ALPHA_VERSION );
	}
	...
	public function set_body_class( $classes ) {
		...
		if ( class_exists( 'WC_Vendors' ) ) {
			...
			if ( is_page( $orders_page_id ) ) {
				$classes[] = 'alpha-wc-vendors alpha-wc-vendors-orders';
			} elseif ( get_the_ID() == $shop_settings_page ) {
				$classes[] = 'alpha-wc-vendors alpha-wc-vendors-shop-settings';
			} elseif ( get_the_ID() == $shop_dashboard_page ) {
				$classes[] = 'alpha-wc-vendors alpha-wc-vendors-dashboard';
			} elseif ( alpha_is_shop() ) {
				$classes[] = 'alpha-wc-vendors alpha-wc-vendor-shop';
			} elseif ( alpha_is_product() ) {
				$classes[] = 'alpha-wc-single-product';
			}
		}
	...
	public function print_sold_by_template( $loop = false ) {
		...
		$sold_by_label = alpha_get_option( 'sold_by_label' ) ? alpha_get_option( 'sold_by_label' ) : get_option( 'wcvendors_label_sold_by' );
		$sold_by       = wcv_get_sold_by_link( $vendor_id );
		$class         = ! $loop ? 'mt-2' : '';
		?>
		<div class="alpha-sold-by-container <?php echo esc_attr( $class ); ?>">
			<span class="sold-by-label"><?php echo esc_html( $sold_by_label ); ?></span>
		<?php echo alpha_escaped( $sold_by ); ?>
		</div>
		<?php
	}
	...
	public function register_sidebar() {
		...
		register_sidebar(
			array(
				'name'          => esc_html__( 'WC Vendors Store Sidebar', 'alpha' ),
	...
	/**
	 * Change Wc Vendor template path.
	 *
	 ...
	 * @since 1.0
	 */
	public function correct_template( $template, $template_name, $args = array(), $template_path = '', $default_path = '' ) {
		if ( false !== strpos( $template_path, 'wc-vendor' ) ) {
			$plugin_dir = str_replace( '/', '\\', WP_PLUGIN_DIR );
			if ( str_replace( '/', '\\', substr( $template, 0, strlen( $plugin_dir ) ) ) == $plugin_dir ) {
				$framework_template_path = ALPHA_PATH . '/framework/' . ALPHA_PART . '/' . str_replace( '_', '-', $template_path . $template_name );
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_wcv_format_store_social_icons' ) ) {
	 ...
	 */
	function alpha_wcv_format_store_social_icons( $vendor_id, $hidden = array() ) {
		ob_start();
		...
		foreach ( wcv_get_social_media_settings() as $key => $setting ) {
			...
			?>
			<li>
				<a href="<?php printf( $setting['url_template'], $value ); ?>" class="social-icon stacked social-<?php echo esc_attr( $setting['w_class'] ); ?>" target="_blank">
					<i class="fab fa-<?php echo esc_attr( $setting['w_icon'] ); ?>"></i>
				</a>
			</li>
			<?php
		}
		...
		return '<ul class="social-icons">' . alpha_escaped( $list ) . '</ul>';
	}
}
