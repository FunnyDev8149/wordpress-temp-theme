	...
	*/
	class Alpha_Optimize_Wizard extends Alpha_Base {
		...
		public function add_admin_menu() {
			add_submenu_page( 'alpha', esc_html__( 'Optimize Wizard', 'alpha' ), esc_html__( 'Optimize Wizard', 'alpha' ), 'manage_options', $this->page_slug, array( $this, 'view_optimize_wizard' ), 3 );
		}
		...
		public function view_optimize_wizard() {
			...
			$title        = array(
				...
				'desc'  => ALPHA_DISPLAY_NAME . esc_html( ' optimize wizard will help you configure proper website with optimum resources and peak efficiency.', 'alpha' ),
			);
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => -1,
						'message'       => esc_html__( 'Activating Plugin', 'alpha' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => -1,
						'message'       => esc_html__( 'Updating Plugin', 'alpha' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => -1,
						'message'       => esc_html__( 'Installing Plugin', 'alpha' ),
					);
					break;
				}
			}
		...
		public function init_wizard_steps() {
			$this->steps = array(
				...
				'performance' => array(
					'name'    => esc_html__( 'Peformance', 'alpha' ),
		...
		public function view_resources() {
			// alpha_compile_dynamic_css( 'optimize', get_theme_mod( 'used_elements' ) );
		...
		public function ajax_optimize_resources() {
			...
			// alpha_compile_dynamic_css( 'optimize', $elements );
