...
defined( 'ABSPATH' ) || die;
?>
<h2 style="margin-bottom: 20px"><?php esc_html_e( 'Performance', 'alpha' ); ?></h2>

<form method="post" class="alpha_submit_form">

	<h3><?php esc_html_e( '1. Optimize Mobile', 'alpha' ); ?></h3>
	...
	<p style="margin: 0 0 20px;"><?php esc_html_e( 'Use scrollbar instead of carousel for only products and posts in mobile.', 'alpha' ); ?></p>
	<?php
		/**
	...
	<h3><?php esc_html_e( '2. Serve Fonts', 'alpha' ); ?></h3>
	<h4 class="sub-title">
		<?php esc_html_e( '- Preload Webfonts', 'alpha' ); ?>
	</h4>
	...
	<h3><?php esc_html_e( '3. Asynchronous Scripts', 'alpha' ); ?></h3>
	...
	<label class="checkbox checkbox-inline">
		<input type="checkbox" name="resource_idle_run" <?php checked( alpha_get_option( 'resource_idle_run' ) ); ?>> <?php esc_html_e( 'Only necessary JS at loading', 'alpha' ); ?>
	</label>
	<p><?php esc_html_e( 'While page is loaded, there exists a lot of unnecessary javascripts running during initialization. If they works in idle time and only necessary ones runs while loading time, page speed will be faster.', 'alpha' ); ?></p>
