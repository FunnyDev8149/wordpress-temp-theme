...
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Import extends WP_Importer {
		...
		function import_end() {
			...
			echo '<p>' . esc_html__( 'All done.', 'alpha' ) . ' <a href="' . esc_url( admin_url() ) . '">' . esc_html__( 'Have fun!', 'alpha' ) . '</a>' . '</p>';
			echo '<p>' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'alpha' ) . '</p>';
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				printf( alpha_strip_script_tags( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'alpha' ) ), esc_html( $file['file'] ) );
				echo '</p>';
				return false;
			}
	...
	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'alpha' ); ?>" /></p>
			...
			foreach ( $this->posts as $post ) {
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					printf( esc_html__( '%1$s &#8220;%2$s&#8221; already exists.', 'alpha' ), $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
					echo '<br />';
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( is_wp_error( $http_response ) ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'alpha' ) );
			}
			...
			if ( '200' != $response['code'] ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote server returned error response %1$d %2$s', 'alpha' ), esc_html( $response['code'] ), get_status_header_desc( $response['code'] ) ) );
			}
			...
			if ( ( ! empty( $max_size ) && $filesize > $max_size ) || ( ! empty( $max_size ) && isset( $headers['content-length'] ) && $headers['content-length'] > $max_size ) ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'alpha' ), size_format( $max_size ) ) );
			}
