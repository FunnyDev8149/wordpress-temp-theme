...
defined( 'ABSPATH' ) || die;
?>
<h2><?php printf( esc_html__( 'Setup %s Child Theme (Optional)', 'alpha' ), ALPHA_DISPLAY_NAME ); ?></h2>
<p>
	<?php printf( esc_html__( 'If you are going to make changes to the theme source code please use a %1$s rather than modifying the main theme HTML/CSS/PHP code. %2$s This allows the parent theme to receive updates without overwriting your source code changes. Use the form below to create and activate the Child Theme.', 'alpha' ), '<a href="https://codex.wordpress.org/Child_Themes" target="_blank">' . esc_html__( 'Child Theme', 'alpha' ) . '</a>', '<br>' ); ?>
</p>
<?php
if ( is_child_theme() ) {
	?>
	...
	<?php if ( ! isset( $_REQUEST['theme_name'] ) ) { ?>
		...
		<form method="POST">
			<div class="child-theme-input" style="margin-bottom: 30px;">
				<label style="font-weight: bold;margin:35px 0 5px; display: block;" for="child-theme"><?php esc_html_e( 'Child Theme Title:', 'alpha' ); ?></label>
			...
			<p class="alpha-admin-panel-actions">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-light button-large button-next button-icon-hide"><?php esc_html_e( 'Skip this step', 'alpha' ); ?></a>
